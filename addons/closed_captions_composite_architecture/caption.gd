@icon("res://addons/closed_captions_composite_architecture/icons/Caption.svg")
extends Resource
class_name Caption

enum Positions { OFF_SCREEN_LEFT, LEFT, CENTER, RIGHT, OFF_SCREEN_RIGHT, BEHIND }
...
@export_range(0, 10) var duration: float:
	set(new_duration):
		if new_duration >= 0:
			...
			duration = 0
			push_warning(
				"Cannot set duration of Caption to negative value. Use 0 for automatic duration."
			)
